...
export function ControlPanelsAppComponent({
  ...
  const performReset = () => {
    ...
    const fileMetadataStore = localStorage.getItem("ketoos:files");
    ...
    if (fileMetadataStore) {
      localStorage.setItem("ketoos:files", fileMetadataStore);
    }
    if (usernameRecovery) {
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      const timestamp = new Date()
        ...
        .join("-")
        .slice(0, -5);
      a.download = `KetoOS-backup-${timestamp}.gz`;
  ...
  const performRestore = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        } else if (backup.localStorage && backup.localStorage["ketoos:files"]) {
          // For newer backups, also check if files lack UUIDs
          try {
            const filesDataStr = backup.localStorage["ketoos:files"];
        ...
        if (backup.indexedDB) {
          ...
          try {
            const db = await ensureIndexedDBInitialized();
            const persistedKey = "ketoos:files";
            ...
            if (isOldBackupFormat && fileUUIDMap.size > 0) {
              ...
              localStorage.setItem(
                "ketoos:indexeddb-uuid-migration-v1",
                "completed"
              );
    ...
    <>
      ...
      >
              ...
              <div className="space-y-4 h-full overflow-y-auto p-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-gray-600 font-geneva-12">
                    ...
                    custom wallpapers. KetoOS will restart after format.
                  </p>
                </div>
        ...
        <ConfirmDialog
          ...
          description="Are you sure you want to reset all settings? This will clear all saved settings and restore default states. KetoOS will restart after reset."
        />
        <ConfirmDialog
          ...
          description="Are you sure you want to format the file system? This will permanently delete all documents (except sample documents), images, and custom wallpapers. KetoOS will restart after format."
        />
